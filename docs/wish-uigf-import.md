...
常驻池新增 5 星的 `resultType` 会设为 `off-banner`；限定池新增 5 星的 `resultType` 会设为 `unknown`，需要后续手动确认是否为 `up` 或 `off-banner`。
...
- 限定池新增 5 星的 `resultType` 是否应改为 `up` 或 `off-banner`。
- 限定角色池是否需要补充 `capturingRadiance`。
- 限定池 5 星是否需要补充 `pullVersion`。
- 4 星冲突是否符合预期。
